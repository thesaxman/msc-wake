...
turbine1 = make_turbine(wp, 0.0, gamma_deg=-15.0) # dynamic yaw, upstream
turbine2 = make_turbine(wp, 5.0, gamma_fn=sinusoid_gamma_t) # static baseline, downstream
...
if __name__ == "__main__":
    ...
    series = with_field(
        WakeSeries(sp.x_grid, sp.ts, u1_xt, u2_xt, yc_xt, wp,
                   label_fn=yaw_label_fn([tb.wp for tb in [turbine1, turbine2]], sp.ts)),
        y_grid,
    )
